#!/usr/bin/env python3
"""
ANIMALCAM_Area_preroll_multiclass.py (Raspberry Pi OS)

Animal presence recorder (non-bird animals):
- Runs MobileNet-SSD inference on the lores stream (Caffe model).
- Starts recording ONLY when a target animal class is detected with sufficient confidence.
...
TARGET_CLASSES = {"cat", "cow", "dog", "horse", "sheep"}  # non-bird animals in this model
...
def pick_out_dir() -> str:
    ...
    # Hardened: require MOUNT to be an actual mount point, not just an existing directory.
    # This prevents accidentally writing to the root filesystem under /media/... when the disk
    # is not mounted.
...
class ClipMeta:
    ...
    min_conf: float = ANIMAL_MIN_CONF
...
def detect_targets_on_lores(net, lo_bgr: np.ndarray) -> Tuple[bool, Optional[str], float, Optional[Tuple[int,int,int,int]]]:
    ...
    seen = best_conf >= ANIMAL_MIN_CONF and best_bbox is not None and best_label is not None
...
def setup_camera_with_overlay(latest_det: Dict[str, Any]) -> Tuple[Picamera2, H264Encoder, CircularOutput]:
    ...
    def pre_callback(request):
        with MappedArray(request, "main") as m:
            ...
            cv2.putText(img, "ANIMALCAM (area)", (16, 30),
            ...
            label = str(latest_det.get('label') or 'animal')
            ...
            if seen and latest_det.get("bbox") is not None:
                ...
                det_label = str(latest_det.get('label') or 'animal')
...
def finalize_clip(base: str, meta: ClipMeta, conf_values: List[float], duration_s: float, note: str = "") -> None:
    ...
    try:
        ...
        if discard:
            ...
            append_event(base, f"{local_now_iso()} | ANIMAL_EXIT | clip={meta.clip_name} | dur={duration_s:.2f}s {note}".rstrip())
...
def main() -> None:
    ...
    def start_clip(with_preroll: bool, trigger_label: str, note: str = "") -> None:
        ...
        append_event(base, f"{local_now_iso()} | ANIMAL_ENTER | clip={clip_name} {note}".rstrip())
        ...
        while True:
            ...
            if recording and current_meta is not None:
                ...
                if seg_len >= MAX_CLIP_SEC:
                    ...
                    keep_label = current_meta.trigger_class or "animal"
            ...
            if (not recording) and now >= cooldown_until and seen_streak >= START_FRAMES:
                start_clip(with_preroll=True, trigger_label=label or "animal")
                continue
